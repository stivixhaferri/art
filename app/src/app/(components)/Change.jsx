...
const Change = ({slug , rate}) => {
  ...
  const handleOtherSubmit = async () => {
    try{
      const res = await axios.get('http://localhost:8000/api/disable' , {
        headers: {
          'token': slug
        },
        cache: 'no-store'
  ...
  const handleEnable = async () => {
    try{
      const res = await axios.get('http://localhost:8000/api/enable' , {
        headers: {
          'token': slug
        },
        cache: 'no-store'
  ...
  const handleSubmit = async () => {
    ...
    try{
      const res = await axios.post('http://localhost:8000/api/reserve' ,{id: slug , date:date});
